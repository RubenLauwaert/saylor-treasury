...
def extract_namespaces(xbrl_string):
    ...
    namespaces = {
        prefix[1:] if prefix else "default": uri for prefix, uri in namespace_pattern
    }
...
class Parser10QXBRL:
    ...
    def __init__(self, xbrl_url: str, xbrl_string: str, ticker: str):
        ...
        self.bitcoin_tag_keywords: List[str] = [
            "Bitcoin",
            "Crypto",
            "BTC",
            "DigitalAsset",
    ...
    def _get_entity_defined_tags(self) -> set[str]:
        ...
        entity_defined_tags = set(
            [
                element.tag.split("}")[1]
                for element in self.tree.iter()
                if self.entity_namespace in element.tag.lower()
            ]
        )
    ...
    def _get_context_for(self, context_id: str) -> dict[str, str]:
        ...
        context_elements: List[ET.Element] = [
            element for element in self.tree.iter() if "context" in element.tag
        ]
        context = {}
            ...
            if context.attrib.get("id") == context_id:
                ...
                if entity is not None:
                    ...
                    if segment is not None:
                        explicit_member = segment.find(
                            f"{self.xbrldi_ns}explicitMember"
                        )
    ...
    async def extract_bitcoin_holdings(
        self,
    ) -> tuple[str, List[BitcoinHoldingsStatement]]:
        """
        Extract Bitcoin holdings from the XBRL file.
        """
        bitcoin_holdings = []
        ai_result_bitcoin_holdings = (
            await XBRL_Extractor().extract_total_bitcoin_holdings_tag(self.bitcoin_tags)
        )
        ...
        if ai_predicted_tag in self.bitcoin_tags and ai_confidence > 0.8:
            ...
            for element in elements:
                if element.text is not None:
                        ...
                        # Now we have a "BTC" unit and decimals, now we need to be carful
                            ...
                            try:
                                ...
                                # This is common for holders that have a low amount of bitcoin and want to report it as an integer
                                    ...
                                    value = (
                                        float(element.text) * 10**decimals
                                        if element.text is not None
                                        else 0
                                    )
                            except:
                    ...
                    bitcoin_holding_statement = BitcoinHoldingsStatement(
                        amount=value,
                        report_date=date,
                        unit=unit,
                        tag=ai_predicted_tag.split("}")[1],
                    )
                    ...
                    if coin_context is not None:
                        if (
                            "bitcoin" in coin_context.lower()
                            or "btc" in coin_context.lower()
                        ):
    ...
    async def extract_bitcoin_fair_value(
        self,
    ) -> tuple[str, List[BitcoinFairValueStatement]]:
        """
        Extract Bitcoin fair value from the XBRL file.
        """
        bitcoin_fair_value = []
        ai_result_bitcoin_fair_value = (
            await XBRL_Extractor().extract_total_bitcoin_fair_value_tag(
                self.bitcoin_tags
            )
        )
        ...
        if ai_predicted_tag in self.bitcoin_tags and ai_confidence > 0.85:
            ...
            for element in elements:
                if element.text is not None:
                    ...
                    fair_value_statement = BitcoinFairValueStatement(
                        amount=value,
                        report_date=date,
                        unit=unit,
                        tag=ai_predicted_tag.split("}")[1],
