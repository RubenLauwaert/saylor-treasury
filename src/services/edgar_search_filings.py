...
from src.modeling.PublicEntity import PublicEntity, map_to_entity
...
def get_us_btc_companies() -> List[PublicEntity]:
    ...
    headers = {"User-Agent": "MyCompanyName - Contact: your_email@example.com"}
    ...
    if response.status_code == 200:
        ...
        public_entities = [
            PublicEntity.from_cik(hits.source.get_cik_number())
...
def get_foreign_btc_companies() -> List[PublicEntity]:
    ...
    if response.status_code == 200:
        ...
        unique_companies = {
            company.cik: company
            for hits in efts_response.hits.hits
            if (company := map_to_entity(hits.source)) and company.cik
        }
