...
class ApiThrottler:
    ...
    async def throttle_requests(
        ...
        ],  # List of async functions returning type T
        batch_size: int = 9,
        delay: float = 1.05,
        ...
        for i in range(0, len(request_funcs), batch_size):
            ...
            results.extend(
                await asyncio.gather(*[func() for func in batch])
            )  # Execute batch
    ...
    @staticmethod
    async def throttle_openai_requests(
        request_funcs: List[Callable[[], Awaitable[T]]]
    ) -> List[T]:
