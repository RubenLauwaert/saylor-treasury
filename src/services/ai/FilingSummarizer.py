...
from modeling.bitcoin_acquisition.BitcoinPurchase import BitcoinPurchase
...
class FilingSummarizer:
    ...
    async def summarize_filings_optimized(self, sec_filings: List[SEC_Filing]) -> List[SEC_Filing]:
        tasks = [self.summarize_filing_optimized(filing) for filing in sec_filings]
        return await asyncio.gather(*tasks)


    
    async def summarize_filing_optimized(self, sec_filing: SEC_Filing) -> SEC_Filing:
