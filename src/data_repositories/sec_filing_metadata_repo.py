...
class SEC_Filing_Metadata_Repository:
    ...
    logger: Logger
    public_entity_collection: PublicEntityRepository

    def __init__(self, collection: Collection):
        self.collection = collection
        self.logger = logging.getLogger(self.__class__.__name__)
        self.public_entity_repo = PublicEntityRepository(public_entity_collection)
    ...
    def get_filing_metadatas_for_entity(
        ...
        filings = self.collection.find({"company_cik": company_cik}).sort(
            "filing_date", -1
        )
        self.logger.info(
    ...
    def get_8k_filing_metadatas_for(
        self, public_entity: PublicEntity
    ) -> List[SEC_Filing_Metadata]:
        company_cik = public_entity.cik
        ticker = public_entity.ticker
        filings = self.collection.find(
            {"company_cik": company_cik, "form": "8-K"}
        ).sort("filing_date", -1)
        self.logger.info(
    ...
    def get_specific_filing_metadatas_for(
        self, ticker: str, form: str
    ) -> List[SEC_Filing_Metadata]:
        entity = self.public_entity_repo.get_entity_by_ticker(ticker)
        company_cik = entity.cik

        filings = self.collection.find({"company_cik": company_cik, "form": form}).sort(
            "filing_date", -1
        )
        self.logger.info(
            f"Retrieved {self.collection.count_documents({'company_cik': company_cik, 'form': form})} {form} filing metadatas for entity with ticker {ticker}."
        )
        return [SEC_Filing_Metadata(**filing) for filing in filings]

    def get_specific_filing_metadatas_for_after_date(
        self, ticker: str, form: str, date: date
    ) -> List[SEC_Filing_Metadata]:
        entity = self.public_entity_repo.get_entity_by_ticker(ticker)
        company_cik = entity.cik
        date_str = date.isoformat()
        ...
        self.logger.info(
            f"Retrieved {self.collection.count_documents({'company_cik': company_cik, 'form': form, 'filing_date': {'$gt': date_str}})} {form} filing metadatas for entity with ticker {ticker} after {date_str}."
        )
        return [SEC_Filing_Metadata(**filing) for filing in filings]
    ...
    def add_filing_metadatas(self, filing_metadatas: List[SEC_Filing_Metadata]):
        operations = [
            UpdateOne(
                {"accession_number": fm.accession_number},
                {"$set": fm.model_dump()},
                upsert=True,
            )
            for fm in filing_metadatas
        ]
        if len(operations) > 0:
            self.collection.bulk_write(operations)
            self.logger.info(
                f"Added or updated {len(filing_metadatas)} filing metadatas."
            )
        else:
    ...
    def update_filing_metadata(self, filing_metadata: SEC_Filing_Metadata):
        ...
        self.logger.info(
            f"Updated filing metadata with ID {filing_metadata.filing_id}."
        )
    ...
    def get_latest_filing_metadata_date_for(
        self, public_entity: PublicEntity
    ) -> Optional[date]:
        company_cik = public_entity.cik
        ticker = public_entity.ticker
        latest_filing = self.collection.find({"company_cik": company_cik}).sort(
            "filing_date", -1
        )
        ...
        if len(latest_filing_list) > 0:
            ...
            self.logger.info(
                f"Latest filing date for entity with ticker {ticker} is {latest_date}."
            )
            return date.fromisoformat(latest_date)
        self.logger.warning(
            f"No filings found for entity with ticker {ticker}. Returning minimal date."
        )
        return date.min
