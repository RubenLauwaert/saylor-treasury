...
class Base_EFTS_Query(BaseModel):
    ...
    from_: Optional[int] = Field(
        None, alias="from", description="The starting index for the search."
    )
...
class Base_Bitcoin_Query(Base_EFTS_Query):
    ...
    startdt: str = Field(
        default=date(2009, 1, 3).isoformat(),
        description="The start date for the query.",
    )
    enddt: str = Field(
        default=date.today().isoformat(), description="The end date for the query."
    )
